...
export class NonceManager {
  ...
  private readonly locks = new Map<string, NonceLock>();
  ...
  private async getLock(chainId: number, address: string): Promise<NonceLock> {
   ...
   */
  async acquire(chainId: number, address: string): Promise<number> {
    const lock = await this.getLock(chainId, address);

    if (lock.queue.length >= MAX_LANES) {
      throw new Error(
        `NonceManager: lane limit (${MAX_LANES}) reached for ${address} on chain ${chainId}`,
      );
    }
   ...
   */
  async sync(chainId: number, address: string): Promise<void> {
    const k = this.key(chainId, address);
    const onChain = await this.provider.getTransactionCount(address, 'pending');
   ...
   */
  async reset(chainId: number, address: string): Promise<void> {
    const k = this.key(chainId, address);
    const onChain = await this.provider.getTransactionCount(address, 'pending');
    if (this.locks.has(k)) {
      const lock = this.locks.get(k)!;
  ...
  private drainQueue(lock: NonceLock): void {
